...
import { PWAUnit} from '../models/models';


export class UnitControllerClient {
	...
	public getByPath(path: string, selectedFields?: string, webIdType?: string) : Observable<PWAUnit> {
		...
		return this.http.get<PWAUnit>(localVarPath, 
		{
	...
	public get(webId: string, selectedFields?: string, webIdType?: string) : Observable<PWAUnit> {
		const localVarPath = this.basePath + '/units/{webId}'
			.replace('{' + 'webId' + '}', String(webId));
	...
	public update(webId: string, unitDTO: PWAUnit) : Observable<any> {
		const localVarPath = this.basePath + '/units/{webId}'
			.replace('{' + 'webId' + '}', String(webId));
		...
		return this.http.patch<any>(localVarPath, unitDTO, 
		{
			params: queryParameters,
			withCredentials: this.withCredentials,
			headers: headers,
