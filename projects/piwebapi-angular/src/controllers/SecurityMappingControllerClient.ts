...
import { PWAItemsSecurityEntry, PWAItemsSecurityRights, PWASecurityEntry, PWASecurityMapping} from '../models/models';


export class SecurityMappingControllerClient {
	...
	public getByPath(path: string, selectedFields?: string, webIdType?: string) : Observable<PWASecurityMapping> {
		...
		return this.http.get<PWASecurityMapping>(localVarPath, 
		{
	...
	public get(webId: string, selectedFields?: string, webIdType?: string) : Observable<PWASecurityMapping> {
		const localVarPath = this.basePath + '/securitymappings/{webId}'
			.replace('{' + 'webId' + '}', String(webId));
	...
	public update(webId: string, securityMapping: PWASecurityMapping) : Observable<any> {
		const localVarPath = this.basePath + '/securitymappings/{webId}'
			.replace('{' + 'webId' + '}', String(webId));
		...
		return this.http.patch<any>(localVarPath, securityMapping, 
		{
			params: queryParameters,
			withCredentials: this.withCredentials,
			headers: headers,
